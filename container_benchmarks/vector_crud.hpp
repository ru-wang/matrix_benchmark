...
struct vector_crud {
    ...
    template<int remain, typename ele_t, typename ...args_t>
        struct emplace_recurse {
            static void impl(std::vector<ele_t>& container,
                    ele_t& ele, args_t... args) {
                container.emplace_back(ele);
    ...
    template<typename ele_t>
        struct emplace_recurse<0, ele_t> {
            static void impl(std::vector<ele_t>& container, ele_t& ele)
            { container.emplace_back(ele); }
        };

    vector_crud() = default;
    vector_crud(int n) { _container_.reserve(n); }
    ...
    void emplace_n_element_at_a_time(int n) {
        for (int i = 0; i < n; ++i)
            _container_.emplace_back((element_t)i);
    }
    ...
    std::vector<element_t> _container_;
};
